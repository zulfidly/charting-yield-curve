...
<template>
    <div v-show="!userDevice.isMobileLandscape" class="w-full p-2 sm:p-3 border border-[--color-border] rounded-lg bg-[--color-background-soft]">
        ...
        <section class="flex gap-1 sm:gap-3">
            <button class="flex gap-4 justify-center items-center mt-3 py-1 bg-[--view-btn] rounded-lg w-full" @click="submitUserOptions('viewTable')">
            ...
            <button :disabled="!disconnectBtn" :class="[!disconnectBtn?'opacity-0':'opacity-100 bg-[--view-btn]']" class="flex gap-4 justify-center items-center mt-3 py-1 rounded-lg w-full" @click="submitUserOptions('viewChart')">
